...
# (c) 2021 Andreas Mueller gnoppix@gmail.com 
# V 10.9.38
#
...
echo "The programs included with the Gnoppix system are free software;" > /etc/legal
echo "the exact distribution terms for each program are described in the " >> /etc/legal
echo "individual files in /usr/share/doc/*/copyright." >> /etc/legal
echo "" >> /etc/legal
echo "Gnoppix comes with ABSOLUTELY NO WARRANTY, to the extent permitted by  " >> /etc/legal
echo "applicable law. " >> /etc/legal


echo "NAME="Gnoppix 11"" > /etc/os-release
echo "VERSION="11"" >> /etc/os-release
echo "ID=eleven" >> /etc/os-release
echo "ID_LIKE="ubuntu debian"" >> /etc/os-release
echo "PRETTY_NAME="Gnoppix 11"" >> /etc/os-release
echo "VARIANT="User Desktop Edition"" >> /etc/os-release
echo "VARIANT_ID=user" >> /etc/os-release
echo "VERSION_ID="20.04"" >> /etc/os-release
echo "HOME_URL="https://www.gnoppix.com/"" >> /etc/os-release
echo "SUPPORT_URL="https://www.gnoppix.com/"" >> /etc/os-release
echo "BUG_REPORT_URL="www.gnoppix.com"" >> /etc/os-release
echo "PRIVACY_POLICY_URL="https://www.ubuntu.com/legal/terms-and-policies/privacy-policy"" >> /etc/os-release
echo "VERSION_CODENAME=focal" >> /etc/os-release
echo "UBUNTU_CODENAME=focal" >> /etc/os-release
echo "" >> /etc/os-release

echo "DISTRIB_ID=gnoppix" >/etc/lsb-release 
echo "DISTRIB_RELEASE=20.04" >>/etc/lsb-release
echo "DISTRIB_CODENAME=focal" >> /etc/lsb-release
echo "DISTRIB_DESCRIPTION="Gnoppix User 11 - Plasma 25th Anniversary Edition" ">> /etc/lsb-release
